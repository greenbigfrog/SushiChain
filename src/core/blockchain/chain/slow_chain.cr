...
module ::Sushi::Core
  class SlowChain
    ...
    alias SlowChain = Array(SlowBlock)
    ...
    getter slowChain : SlowChain = SlowChain.new
    ...
    def restore_from_database(database : Database)
      ...
      push_genesis if @SlowChain.size == 0
    end
    ...
    def replace_chain(_subchain : SlowChain?) : Bool
      return false unless subchain = _subchain
      return false if subchain.size == 0
      return false if @SlowChain.size == 0

      first_index = subchain[0].index

      if first_index == 0
        @SlowChain = [] of SlowBlock
      else
        @SlowChain = @SlowChain[0..first_index - 1]
      end
      ...
      push_genesis if @SlowChain.size == 0
      if database = @database
        database.replace_chain(@SlowChain)
      end
    ...
    def subchain(from : Int64) : SlowChain?
      return nil if @SlowChain.size < from
    ...
    def headers
      @SlowChain.map { |block| block.to_header }
    end
