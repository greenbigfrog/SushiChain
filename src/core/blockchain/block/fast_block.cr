...
module ::Sushi::Core
  class FastBlock
    ...
    def to_s
      debug "Fast block index: #{@index}"
      debug "Fast block transactions: #{@transactions.size}"
      debug "Fast block prev_hash: #{@prev_hash}"
      debug "Fast block timestamp: #{@timestamp}"
      debug "Fast block hash: #{self.to_hash}"
    end
    ...
    # TODO - fast cannot be genesis block
    def valid?(blockchain : Blockchain, skip_transactions : Bool = false) : Bool
      valid_as_latest?(blockchain, skip_transactions)
    end

    # TODO - fast cannot be genesis block
    #      - need fast transaction pool
